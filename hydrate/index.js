...
var reactEditor = require('@magnolia/react-editor');
...
const { REACT_APP_MGNL_HOST: REACT_APP_MGNL_HOST$1, REACT_APP_MGNL_API_PAGES, REACT_APP_MGNL_API_TEMPLATES, SITE_COUNTRY, SITE_NAME } = process.env;

const getPagePath = (context, settings) => {
  const languages = context.locales;
  const nodeName = `/${SITE_NAME}-${SITE_COUNTRY}`;
  let path = (REACT_APP_MGNL_HOST$1) + REACT_APP_MGNL_API_PAGES +
    nodeName +
    context.resolvedUrl.replace(new RegExp('(.*' + nodeName + '|.html)', 'g'), '');
  const currentLanguage = context.query.mgnlPreview === 'false' ? getCurrentLanguage(path, context.locales) : context.locale;
...
async function getPage(context, settings) {
  ...
  if (context.query.mgnlPreview === 'false') {
    const templateDefinitionsRes = await fetch(
      REACT_APP_MGNL_HOST$1 + REACT_APP_MGNL_API_TEMPLATES + '/' + page['mgnl:template']
    );
...
const getMgnlApp = async (config) => {
  ...
  console.log('mgnlURL', `${host || REACT_APP_MGNL_HOST}${REACT_APP_MGNL_API_APP}${endpoint}/${site}-${country}/setting?lang=${lang}`);
  const content = await fetch(`${host || REACT_APP_MGNL_HOST}${REACT_APP_MGNL_API_APP}${endpoint}/${site}-${country}/setting?lang=${lang}`).then(r => r.json());
  return (resType != '' && resType == 'JCR') ? calulateNodeData(content) : { ...content }

};

Object.defineProperty(exports, 'EditableArea', {
  enumerable: true,
  get: function () { return reactEditor.EditableArea; }
});
Object.defineProperty(exports, 'EditablePage', {
  enumerable: true,
  get: function () { return reactEditor.EditablePage; }
});
exports.getMgnlApp = getMgnlApp;
exports.getPage = getPage;

var Mgnl = /*#__PURE__*/Object.freeze({
  __proto__: null
});
